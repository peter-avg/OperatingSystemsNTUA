                             ...
                             struct page **res_page)
{
	...
	*res_page = NULL;
	...
	for (i=0; i < npages; i++) {
		/* ? */
        // Ανάγνωση σελίδων ενός καταλόγου
        page = ext2_get_page(dir,i,0);
        
        // Αν η ανάγνωση αποτύχει
        if (IS_ERR(page))
            return ERR_CAST(page);

        // Επιστροφή της διεύθυνσης του χώρου διευθύνσεων του πυρήνα
        // για την δοθείσα σελίδα
        kaddr = page_address(page);

        // Επιστροφή του τελευταίου χρησιμοποιούμενου byte της σελίδας
        // page_nr του inode (για σελίδες που περιέχουν directory entries)
        de = (ext2_dirent *) kaddr;
        kaddr += ext2_last_byte(dir, n) - reclen;
        while((char *) de <= kaddr) {
            // Μηδενικό μηκος directory entry
            if(de->rec_len == 0) {
