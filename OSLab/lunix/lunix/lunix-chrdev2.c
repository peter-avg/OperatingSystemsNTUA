...
static int lunix_chrdev_state_needs_refresh(struct lunix_chrdev_state_struct *state)
{
	...
	/* ? */
	/* Λογικά βλέπουμε τον σένσορα, και αν αποτύχουμε να δούμε το αποτέλεσμα θέλει refresh */
...
static int lunix_chrdev_open(struct inode *inode, struct file *filp)
{
	/* Declarations */
	/* ? */
	lunix_chrdev_cdev = container_of(inode->i_cdev, struct cdev, cdev); 
	filp->private_data = lunix-chrdev-cdev;
	int ret;
...
int lunix_chrdev_init(void)
{
	...
	ret = register_chrdev_region(dev_no,16,"ttyS1");
